...
export function buildSummaryPrompt(input: {
  ...
  const personalMemoryRule = input.personalMemoryContext?.trim()
    ? `\n- Personal memory context is the logged-in user's accumulated memory from PAST meetings. Use it ONLY to connect this meeting to ongoing work: recognize recurring projects, people, and threads, resolve vague references (for example expand "the migration", "the AX thing", or a bare first name to the project, product, or person the memory identifies), and note genuine continuity or contradiction with what the File Transcript actually says. It must NOT introduce people, decisions, action items, numbers, or any fact that is not present in the File Transcript. If it is irrelevant to this meeting, ignore it completely.
- Resolving WHAT a reference points to (a project name, a product, who a person is) is allowed. Asserting a specific VALUE the transcript did not state is NOT: when the transcript only alludes to a prior decision, date, amount, or number without stating it (for example "the launch date we agreed", "the discount we settled on", "the budget"), you may name the topic being referred to, but you MUST NOT substitute the concrete date, number, amount, or decided value from memory. Keep it as "the previously agreed launch date" or "the agreed discount", never a specific value like "October 14" or "20%" unless the File Transcript itself states it. Never restate a memory item as a fact of this meeting unless the File Transcript supports it.`
    : '';
