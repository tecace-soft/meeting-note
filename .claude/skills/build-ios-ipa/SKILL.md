...
- **Never raise the iOS recording bitrate above 48 kbps.** `recording_service.dart` uses `bitRate: Platform.isIOS ? 48000 : 64000` at 16 kHz mono. iOS's AAC-LC encoder rejects 64 kbps at that sample rate and fails *silently*: `isRecording` stays true and the timer runs, but the mic never engages and the file stops at its 28-byte `ftyp` header, so the loss only appears at upload. Measured on iPhone 15 Pro Max / iOS 26.6 — 32 kbps and 48 kbps both record fine, 64 kbps captured nothing in 3 of 3 attempts. Android is unaffected (it uses `MediaRecorder` in `ForegroundRecordingService.kt` with its own bitrate), so a change made "to match web" will silently break iOS only. `RecordingNotifier._watchForCapture` now aborts a silent recording after ~3s instead of letting a whole meeting be lost.
- **iOS version is set in Xcode, not pubspec.** `Info.plist` reads `$(MARKETING_VERSION)` / `$(CURRENT_PROJECT_VERSION)`, so `flutter build --build-name/--build-number` no longer affects iOS. Bump the iOS build number in Xcode (or `xcrun agvtool new-version -all N`) before each TestFlight upload — TestFlight rejects a re-used build number.

  **This is an intentional exception to the repo's mobile versioning convention.** `.github/workflows/tag-mobile-release.yml` treats the pubspec `version:` as the single human-bumped mobile release number and tags `mobile-v<version>` from it; `build_apk.sh` derives Android's versionName/versionCode from the same place. That line tracks **Android only** — a `mobile-v*` tag does not describe what shipped to TestFlight, and bumping pubspec does not move the iOS build. Do not "fix" this by pointing `Info.plist` back at `$(FLUTTER_BUILD_NAME)`: those values live in the gitignored `ios/Flutter/Generated.xcconfig`, which any `flutter pub get` rewrites from pubspec, so an Xcode Archive taken afterwards silently carries the wrong version.
