  ...
  const handleConfirmDeleteNote = async () => {
    ...
    try {
      ...
      setProjects((prev) =>
        prev.map((p) => ({
          ...p,
          notes: (p.notes || []).filter((nid) => String(nid) !== String(removedId)),
        }))
      );
      setSelectedNoteIds((prev) => prev.filter((id) => id !== removedId));
                ...
                {notes.map(note => (
                  ...
                  >
                    <div
                      ...
                      className="grid cursor-pointer grid-cols-[2rem_2.5rem_minmax(0,1fr)_auto] items-center gap-x-2 gap-y-0 py-3 pl-2 pr-3 transition-all sm:gap-x-2.5 sm:py-3.5 sm:pl-3 sm:pr-4"
                      style={{ backgroundColor: expandedNoteId === note.id ? 'var(--bg-secondary)' : undefined }}
                    >
                      <div
                        className="flex h-10 w-full max-w-[2rem] shrink-0 items-center justify-center"
                      ...
                      >
                        <input
                          ...
                          className="h-4 w-4 shrink-0 appearance-none rounded border checked:appearance-auto"
                          style={{ borderColor: 'var(--border)', backgroundColor: 'transparent' }}
                        />
                      </div>
                      <div
