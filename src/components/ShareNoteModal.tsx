...
interface ShareNoteModalProps {
  ...
  onClose: () => void;
  onShared?: (noteId: string, sharedUserIds: string[]) => void;
}
...
const ShareNoteModal: React.FC<ShareNoteModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onShared?.(noteId, selectedIds);
      onClose();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Could not share this note.');
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="mb-4 flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
              {noteTitle?.trim()
                ? `Select TecAce contacts who should have access to "${noteTitle.trim()}".`
                : 'Select TecAce contacts who should have access to this note.'}
            </p>
          </div>
          <button
            type="button"
