...
function preferDecryptedText(decrypted: string | null | undefined, fallback: string | null | undefined): string | null | undefined {
  return typeof decrypted === 'string' && decrypted.trim() ? decrypted : fallback;
}

export function decryptNoteForMcp<T extends NoteRow>(note: T): T {
  if (!isEncryptedNotePayload(note.encrypted_payload)) return note;

  const ownerUserId = note.user_id?.trim();
  if (!ownerUserId) {
    return { ...note, decryption_error: 'Encrypted note is missing an owner user id.' };
  }

  const authContext = getScopedAuthContext();
  const secret = authContext.userId === ownerUserId ? authContext.noteEncryptionSecret : undefined;
  if (!secret) {
  ...
  try {
    ...
    return {
      ...note,
      summary: preferDecryptedText(payload.summary, note.summary),
      ...
      transcription: preferDecryptedText(payload.transcription, note.transcription),
