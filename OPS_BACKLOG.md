...
- **SHIPPED 2026-08-13** (commit `9c32e31`, deployed from `mcp-server`). Added the FIRST MCP write tool `add_note_to_project` (note is attributed to the authenticated **scoped user** — no speaker ID; the actor is the caller, not a transcript speaker). The existing `add_accessible_note_to_project` RPC derives the caller from `auth.jwt()->>'sub'`, which is NULL under the MCP's service_role — so a new **service_role-only** RPC `add_note_to_project_for_user(note, project, user)` (migration `20260813180000`, applied to prod via SQL Editor) takes the user id explicitly and re-checks the SAME ownership rules (project must be owned by the user, note owned-or-shared). Granted to service_role only, so a normal authenticated user cannot pass an arbitrary user id.
  ...
  - **⚠️ MULTI-ACCOUNT FRAGMENTATION found while testing (sprint-worthy):** the user records/uses the app under `andrewyoo@tecace.com` (oid `31d79bfe`, = notes + MCP scope + memory) but had created the old "Meeting Note Development" project under a DIFFERENT web login (oid `d9eb0f3d`). So the note and project lived on different identities → add refused. Fix was to create the project under the SAME account (new project 21 "meeting note", owned by `31d79bfe`) → add succeeded. **Implication beyond this note: memory / index / speaker-accumulation are ALL per-user-id, so using two logins silently splits a single person's accumulated context in half.** Product takeaway: enforce/encourage ONE identity per person (phone + web same login); consider surfacing "you have data under another account" or an account-merge path. Follow-up (optional): symmetric `remove_note_from_project` tool.
