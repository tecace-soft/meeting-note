...
- **Phases:** F5.0 identity→insight/memory (closes owner gap; do first) · F5.1 server-side auto-identify at ingest (hybrid) · F5.2 richer context (user_memory + profiles + MS attendees + topic overlap) · F5.3 cross-note accumulation loop + F8 speaker-id accuracy surface (time-lagged eval the boss asked for) · F5.4 acoustic embeddings DEFERRED.
